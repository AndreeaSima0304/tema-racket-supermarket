...
(define (tt+ C minutes)
  (define newtime (+ minutes (counter-tt C)))
  (define C2 (struct-copy counter C [tt newtime]))
  (match C
...
(define (min-tt counters)
   (cond
     ...
     [(and (= (counter-tt (car counters)) (cdr (min-tt (cdr counters))))
           (> (counter-index (car counters)) (cdr (min-tt (cdr counters)))))
      (cons (counter-index (car counters)) (counter-tt (car counters)))
     ]
...
(define (add-to-counter C name n-items)
  (define new-customer (cons name n-items))
  (define new-queue (append (counter-queue C) (list new-customer)))
  (define new-customer-time (+ (counter-tt C) n-items))
  (define C2 (struct-copy counter C [tt new-customer-time] [queue new-queue]))
  (match C
    [(counter index tt queue)
     C2])
)
...
(define (serve requests C1 C2 C3 C4)
  (define (new-time C n-items) (+ (counter-tt C) n-items))
  (define (new-customer name n-items) (list (cons name n-items)))
  (define (new-queue C name n-items) (append (counter-queue C) (new-customer name n-items)))
  ...
  (if (null? requests)
      ...
      (match (car requests)
        [(list 'delay index minutes)
         (cond
           [(equal? index 1) (define delayed-counter (make-counter 1 (+ minutes (counter-tt C1)) (counter-queue C1)))
                             (serve (cdr requests) delayed-counter C2 C3 C4)]
           [(equal? index 2) (define delayed-counter (make-counter 2 (+ minutes (counter-tt C2)) (counter-queue C2)))
                             (serve (cdr requests) C1 delayed-counter C3 C4)]
           [(equal? index 3) (define delayed-counter (make-counter 3 (+ minutes (counter-tt C3)) (counter-queue C3)))
                             (serve (cdr requests) C1 C2 delayed-counter C4)]
           [(equal? index 4) (define delayed-counter (make-counter 4 (+ minutes (counter-tt C4)) (counter-queue C4)))
                             (serve (cdr requests) C1 C2 C3 delayed-counter)]
         )
        ]
        [(list name n-items)
         (cond
           [(and (equal? (car (min-tt (list C1 C2 C3 C4))) 1) (<= n-items ITEMS))
            (define updated-counter (make-counter 1 (new-time C1 n-items) (new-queue C1 name n-items)))
            (serve (cdr requests) updated-counter C2 C3 C4)]
           [(equal? (car (min-tt (list C1 C2 C3 C4))) 2)
            (define updated-counter (make-counter 2 (new-time C2 n-items) (new-queue C2 name n-items)))
            (serve (cdr requests) C1 updated-counter C3 C4)]
           [(equal? (car (min-tt (list C1 C2 C3 C4))) 3)
            (define updated-counter (make-counter 3 (new-time C3 n-items) (new-queue C3 name n-items)))
            (serve (cdr requests) C1 C2 updated-counter C4)]
           [(equal? (car (min-tt (list C1 C2 C3 C4))) 4)
            (define updated-counter (make-counter 4 (new-time C4 n-items) (new-queue C4 name n-items)))
            (serve (cdr requests) C1 C2 C3 updated-counter)]
           [(and (equal? (car (min-tt (list C1 C2 C3 C4))) 1) (> n-items ITEMS))
            (cond
              [(equal? (car (min-tt (list C2 C3 C4))) 2)
               (define updated-counter (make-counter 2 (new-time C2 n-items) (new-queue C2 name n-items)))
               (serve (cdr requests) C1 updated-counter C3 C4)]
              [(equal? (car (min-tt (list C2 C3 C4))) 3)
               (define updated-counter (make-counter 3 (new-time C3 n-items) (new-queue C3 name n-items)))
               (serve (cdr requests) C1 C2 updated-counter C4)]
              [(equal? (car (min-tt (list C2 C3 C4))) 4)
               (define updated-counter (make-counter 4 (new-time C4 n-items) (new-queue C4 name n-items)))
               (serve (cdr requests) C1 C2 C3 updated-counter)]
            )
           ]
