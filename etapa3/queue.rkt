...
(define (dequeue q)
  (cond
    [(not (null? (queue-left q))) (make-queue (cdr (queue-left q)) (queue-right q) (sub1 (queue-size-l q)) (queue-size-r q))]
    [else (make-queue (cdr (append (queue-left q) (reverse (queue-right q)))) '() (sub1 (queue-size-r q)) 0)]
  )
)
...
(define (top q)
  (cond
    [(null? (queue-left q)) (car (reverse (queue-right q)))]
